...
simulation_replicate_bugs <- function(true.beta0, true.beta1, Module, Trt, cutoff.theta){
  ...
  TDmodule =c(1,1,1,1,1); GDmodule=c(1,2,3,4,5); Dmod=c(1,1)
  
  data <- list("ngroups", "npatients", "ntreat", "y", "TDmodule", "Dmod", "GDmodule", "Module", "Trt", "prior.beta1", "prior.sd.beta1",
               "s0", "lSlab", "uSlab", "spike","true.beta0", "prior.mu.beta0", "prior.sig.HN", "cutoff.theta")
  
  parameters <- c("theta", "pCat", "pCat2","beta1")
  
  MCMCSim_model2 <- jags(data = data, inits = inits, parameters.to.save = parameters, model.file = "RandBasket_cont_data_model2.txt", 
                         n.chains = 2, n.burnin = 3000, n.iter = 13000, working.directory = "C:/Users/Luke Ondijo/Rbasket/code/JAGS")
  
  return(summarystats = MCMCSim_model2$BUGSoutput$summary)
}
...
pLapply_BUGS_replicates_RB_trial_mod2_scen1 <- parLapply(cl=c1, 1:niterations, function(i){simulation_replicate_bugs(
...
saveRDS(pLapply_BUGS_replicates_RB_trial_mod2_scen1, "results/JAGS_replicates_RB_trial_mod2_scen1")